...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('am.sticker-type.update', encrypt($stickerType->id)) }}" method="POST"
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="category_id" name="category_id">
                        ...
                        @foreach ($categories as $id => $name)
                            <option value="{{ $id }}"
                                {{ old('category_id', $stickerType->category_id) == $id ? 'selected' : '' }}>
                                {{ $name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="name" name="name"
                        value="{{ old('name', $stickerType->name) }}">
                    @error('name')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="slug" name="slug"
                        value="{{ old('slug', $stickerType->slug) }}">
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" id="description" name="description" rows="4">{{ old('description', $stickerType->description) }}</textarea>
                    @error('description')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <x-file-upload name="thumbnail" type="image" existingFile="{{ $stickerType->thumbnail }}"
                        maxSize="2" />
